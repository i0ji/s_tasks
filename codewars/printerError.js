...
four times color b, one time c0olor h then one time color a...
...
of errors and the denominator the length of the control string.
...
function printerError(s) {
    ...
    k = Array.from(s)
    z = k.filter(el => el > `m`)
    return console.log(`${z.length}/${s.length}`)
}
printerError(`aaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbmmmmmmmmmmmmmmmmmmmxyz`)

function printerError(s) {
    error =  Array.from(s).filter(element => element > `m`) 
